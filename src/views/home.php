                ...
                <div class="card h-100 shadow-sm">
                    ...
                    <div class="card-body">
                        <span class="badge bg-secondary mb-2">
                            <?= htmlspecialchars($annonce['category_name'] ?? 'Divers') ?>
                        </span>

                        <h5 class="card-title">
                            <?= htmlspecialchars($annonce['title'] ?? 'Sans titre') ?>
                        </h5>
                        
                        <p class="card-text text-primary fw-bold">
                            <?= htmlspecialchars($annonce['price'] ?? '0') ?> €
                        </p>
                        
                        <p class="card-text text-truncate">
                            <?= htmlspecialchars($annonce['description'] ?? 'Aucune description') ?>
                        </p>
                    </div>
                    
                    <div class="card-footer bg-white border-top-0">
                        <a href="index.php?page=detail&id=<?= $annonce['id'] ?>" class="btn btn-outline-primary w-100">
                            Voir détails
                        </a>
                    </div>
                </div>
